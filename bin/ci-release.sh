...
set-up-ssh() {
  ...
  git config --global user.name "clove bot"
...
npx github-changes -o clove -r vscode-clove-syntax --no-merges -t "clove Syntax (official) Changelog" -k $GITHUB_TOKEN
git commit -am "Update release notes for $TRAVIS_TAG"
git remote remove origin
git remote add origin git@github.com:clove/vscode-clove-syntax.git
